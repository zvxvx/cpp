...
int main() {
  ColorBlob cBlob(
      2, 2, Color());  // creates with the default color, width, and height
  cout << "cBlob = " << endl;
  ...
  cout << cBlobThree << endl;
  ColorBlob cBlobFour =
      cBlobOne - cBlobTwo;  // subtracts one ColorBlob from the other
  cout << "cBlobFour = " << endl;
  cout << cBlobFour << endl;
  Color cColor(0.5, 0.4, 0.45);
  ColorBlob cBlobFive =
      cBlobOne * cColor;  // multiples the ColorBlob with cColor
  cout << "cBlobFive= " << endl;
  cout << cBlobFive << endl;

  if (!cBlobFive)
    cout << "It is not black!" << endl;  // if red, green, and blue components
                                         // of color are zero, then returns true
  else
